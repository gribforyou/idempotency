...
/**
 * Unit tests for {@link Lz4Base64Compressor}.
 */
class Lz4Base64CompressorTest {

    private final Lz4Base64Compressor compressor = new Lz4Base64Compressor();
    ...
    void compress_whenLongRepetitiveString_shouldReturnCompressed() {
        ...
        for (int i = 0; i < 100; i++) {
            sb.append("This is a repetitive text block for LZ4 compression test. ");
        }
        ...
        assertTrue(result.matches("[A-Za-z0-9_-]+"), "Результат должен быть URL-safe Base64 строкой");
    ...
    @Test
    void compress_whenVeryLargeString_shouldSignificantlyReduceSize() {
        // Given
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 1000; i++) {
            sb.append("Large data block number ").append(i).append(" for compression testing. ");
        }
        String veryLargeInput = sb.toString();

        // When
        String result = compressor.compress(veryLargeInput);

        // Then
        assertNotNull(result);
        assertTrue(result.length() < veryLargeInput.length(), "Очень большая строка должна быть сжата");
        double ratio = (double) result.length() / veryLargeInput.length();
        assertTrue(ratio < 0.8, "Степень сжатия должна быть заметной (менее 80% от оригинала)");
    }
}